...
MOUNT_DIR = os.environ.get("MOUNT_DIR", None)
BASE_DIR = str(Path(__file__).parents[2])
RES_DIR = os.path.join(BASE_DIR, MOUNT_DIR) if MOUNT_DIR is not None else BASE_DIR
...
try:
    ...
    with open(os.path.join(RES_DIR, "keys", "secret_key.py"), "w") as f:
...
def generate_ssh_keys():
    keys_dir = os.path.join(RES_DIR, "keys")
    ...
    with open(pidfile, "w") as pid:
        ...
        try:
            subprocess.run(
                ["ssh-add {}/*".format(ssh_dir)], shell=True, stderr=subprocess.PIPE
            )
            keys = (
            ...
            if "has no identities" in keys[0]:
                ...
                if not ("id_rsa" in volume_keys and "id_rsa.pub" in volume_keys):
                    ...
                    subprocess.run(
                        [
                            'ssh-keygen -b 4096 -t rsa -f {}/id_rsa -q -N ""'.format(
                                ssh_dir
                            )
                        ],
                        shell=True,
                    ...
                    shutil.copyfile(
                        "{}/id_rsa".format(ssh_dir), "{}/id_rsa".format(keys_dir)
                    )
                    shutil.copymode(
                        "{}/id_rsa".format(ssh_dir), "{}/id_rsa".format(keys_dir)
                    )
                    ...
                    shutil.copyfile(
                        "{}/id_rsa".format(keys_dir), "{}/id_rsa".format(ssh_dir)
                    )
                    shutil.copymode(
                        "{}/id_rsa".format(keys_dir), "{}/id_rsa".format(ssh_dir)
                    )
...
LOGIN_REDIRECT_URL = "/"
AUTH_LOGIN_NOTE = (
    '<p>Have not registered yet? <a href="/auth/register">Register here</a>.</p>'
)
...
RQ_QUEUES = {
    "default": {"HOST": "localhost", "PORT": 6379, "DB": 0, "DEFAULT_TIMEOUT": "4h"},
    "low": {"HOST": "localhost", "PORT": 6379, "DB": 0, "DEFAULT_TIMEOUT": "24h"},
}
...
DATA_ROOT = os.path.join(RES_DIR, "data")
os.makedirs(DATA_ROOT, exist_ok=True)
SHARE_ROOT = os.path.join(RES_DIR, "share")
os.makedirs(SHARE_ROOT, exist_ok=True)
MODELS_ROOT = os.path.join(RES_DIR, "models")
...
#BASE_URL = 'https://tagging.matrix.one'
